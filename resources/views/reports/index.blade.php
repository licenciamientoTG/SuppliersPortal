...
<section class="reports-month-chart" aria-labelledby="monthlyStatusTitle"><div class="reports-chart-head"><div><span class="reports-kicker">Seguimiento operativo</span><h2 id="monthlyStatusTitle" class="reports-chart-title mb-1">Requisiciones por estatus y departamento</h2><p class="reports-chart-subtitle mb-0">Creadas durante {{ $monthlyStatus['period'] }}.</p></div><div class="reports-chart-total"><span class="d-block small text-muted fw-normal">Total del mes</span>{{ $monthlyStatus['total'] }}</div></div>@if($monthlyStatus['total'])<div id="requisitionStatusChart" class="reports-chart-canvas" aria-label="Gráfica de requisiciones por estatus y departamento"></div>@else<div class="reports-chart-empty"><i class="ti ti-chart-bar-off me-1"></i>No hay requisiciones creadas en el mes en curso.</div>@endif</section>
