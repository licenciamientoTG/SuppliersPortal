...
@section('title', $type->exists ? 'Editar documento' : 'Nuevo documento')
@section('page.title', $type->exists ? 'Editar documento' : 'Nuevo documento')
@section('content')
<form method="POST" action="{{ $type->exists ? route('supplier-document-types.update', $type) : route('supplier-document-types.store') }}">
    @csrf
    @if($type->exists) @method('PUT') @endif
    <div class="card">
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4"><label class="form-label">Clave</label><input class="form-control" name="code" value="{{ old('code', $type->code) }}" required></div>
        ...
        <div class="card-footer d-flex justify-content-end gap-2"><a class="btn btn-light" href="{{ route('supplier-document-types.index') }}">Cancelar</a><button class="btn btn-primary" type="submit">Guardar</button></div>
    </div>
</form>
@endsection
