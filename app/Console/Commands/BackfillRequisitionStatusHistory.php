        ...
        Requisition::with(['rfqs', 'quotationSummaries', 'purchaseOrders.receptions'])->chunkById(100, function ($rows) {
            foreach ($rows as $requisition) {
                $events = [
                    ...
                    ['ADJUDICATION_PENDING_APPROVAL', 'QUOTED', 'PENDING_APPROVAL', $requisition->quotationSummaries->where('approval_status', 'pending')->min('created_at'), null],
                    ['QUOTATION_APPROVED', 'PENDING_APPROVAL', 'APPROVED', $requisition->quotationSummaries->whereNotNull('approved_at')->min('approved_at'), null],
