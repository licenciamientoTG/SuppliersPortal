...
class ReportingService
{
    ...
    public function filters(): array
    {
        return [
            ...
            'suppliers' => DB::table('suppliers')->orderBy('business_name')->get(['id', 'business_name']),
    ...
    private function traceability($from,$to,$f): array {
        ...
        $rows=$q->orderByDesc('r.created_at')->get()->map(function($r){$r->dias_ciclo=$r->recibido?Carbon::parse($r->created_at)->diffInDays($r->recibido):Carbon::parse($r->created_at)->diffInDays(now());return $r;});
    ...
    private function suppliers($from,$to,$f): array { $q=DB::table('purchase_orders as po')->join('suppliers as s','s.id','=','po.supplier_id')->whereNull('po.deleted_at')->whereBetween('po.created_at',[$from,Carbon::parse($to)->endOfDay()]); if(!empty($f['supplier_id']))$q->where('s.id',$f['supplier_id']); $rows=$q->selectRaw("s.business_name as proveedor, COUNT(*) as ordenes, SUM(po.total) as monto, SUM(CASE WHEN po.status='RECEIVED' THEN 1 ELSE 0 END) as recibidas, SUM(CASE WHEN po.status='DELIVERED_PENDING_RECEPTION' THEN 1 ELSE 0 END) as pendientes_recepcion")->groupBy('s.id','s.business_name')->orderByDesc('monto')->get(); return $this->pack(['Proveedor','Órdenes','Monto','Recibidas','Pendientes recepción'],$rows,['Proveedores'=>$rows->count(),'Monto'=>$rows->sum('monto'),'Órdenes'=>$rows->sum('ordenes')]); }
    ...
    private function contracts($from,$to,$f): array { $q=DB::table('contracts as c')->join('suppliers as s','s.id','=','c.supplier_id')->join('companies as co','co.id','=','c.company_id')->whereBetween('c.end_date',[$from,$to]); if(!empty($f['contract_id']))$q->where('c.id',$f['contract_id']); $rows=$q->selectRaw('c.folio, s.business_name as proveedor, co.name as empresa, c.contract_amount as monto_contratado, c.start_date, c.end_date, c.status')->orderBy('c.end_date')->get(); return $this->pack(['Folio','Proveedor','Empresa','Monto contratado','Inicio','Vencimiento','Estatus'],$rows,['Contratos'=>$rows->count(),'Monto contratado'=>$rows->sum('monto_contratado'),'Vencen en 30 días'=>$rows->filter(fn($r)=>Carbon::parse($r->end_date)->between(now(),now()->addDays(30)))->count()]); }
}
